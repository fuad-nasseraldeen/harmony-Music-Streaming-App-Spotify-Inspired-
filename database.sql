...
/**
 * SONGS
 * Table for storing uploaded songs
 */
create table songs (
...
create policy "Allow owner delete." on songs
  for delete using (auth.uid() = user_id);

/**
 * LIKED SONGS
 * Junction table for user liked songs
 */
...
alter table liked_songs
  enable row level security;
create policy "Allow users to view own likes." on liked_songs
  for select using (auth.uid() = user_id);
create policy "Allow users to insert own likes." on liked_songs
  for insert with check (auth.uid() = user_id);
create policy "Allow users to delete own likes." on liked_songs
  for delete using (auth.uid() = user_id);
